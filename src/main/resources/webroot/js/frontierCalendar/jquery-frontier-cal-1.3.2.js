...
(function ($) {
    ...
    var jFrontierCalPlugin = function (
        ...
        this.addAgendaItem = function (scheduledItem) {
            if (scheduledItem.calendarId !== null && scheduledItem.title !== null
                    && scheduledItem.scheduledRoom.startDate !== null && scheduledItem.scheduledRoom.endDate !== null && scheduledItem.scheduledRoom.allDay !== null) {
                // make sure start date comes before end date
                if (DateUtil.secondsDifferenceDirection(scheduledItem.scheduledRoom.startDate, scheduledItem.scheduledRoom.endDate) < 0) {
                    alert("Sorry, you can't create an event that ends before it starts");
                    return;
                }
                calId = stripNumberSign(scheduledItem.calendarId);
                var hashData = new Hashtable();
                if (scheduledItem.scheduledRoom.agendaData !== null) {
                    for (var key in scheduledItem.scheduledRoom.data) {
                        hashData.put(key, scheduledItem.scheduledRoom.data[key]);
                    }
                }
...
//				if(displayProp != null){
                ...
                if (scheduledItem.foregroundColor !== null) {
                    agi.setForegroundColor(scheduledItem.foregroundColor);
                }
//				}
                var calObj = myCalendars.get(calId);
                calObj.addAgendaItem(agi);
